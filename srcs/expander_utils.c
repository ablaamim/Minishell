...
/*   Updated: 2022/08/26 19:17:12 by ablaamim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	expander_utils_initializer(char **argv, char **splitted_variable, char *var_val, t_expander *utils)
{
	...
	utils->len_new_argv = utils->len_old_argv + utils->len_argv_to_append - (utils->first_joined == false) - (utils->last_joined == false) + (utils->has_space == true);
	utils->i_old = 0x0;
	utils->i_new = 0x0;
	utils->i_split = 0x0;
...
{
	if (i > 0 && retrieve_len_array(*splitted_variable) > 1 && argument_is_redirection(argv[i - 1]) == true)
	{
		variadic_error_printer(2, "minishell : %s : ambiguous redirection ", argv[i]);
...
{
	char		**new_argv;
	char		**splitted_variable;
	t_expander	utils;

	splitted_variable = ft_split(var_val, ' ');
	if (verify_if_expansion_is_valid(i, &splitted_variable, cmd->argv) == false)
		return (false);
	expander_utils_initializer(cmd->argv, splitted_variable, var_val, &utils);
	...
	manage_first_join(cmd->argv[i], new_argv, splitted_variable, &utils);
	while (utils.i_split < utils.len_argv_to_append)
		new_argv[utils.i_new++] = ft_strdup(splitted_variable[utils.i_split++]);
	manage_last_join(cmd->argv[i], new_argv, splitted_variable, &utils);
	++utils.i_old;
	while(cmd->argv[utils.i_old] != 0x0)
		new_argv[utils.i_new++] = ft_strdup(cmd->argv[utils.i_old++]);
	new_argv[utils.i_new] = 0x0;
	array_free(&cmd->argv);
	array_free(&splitted_variable);
	garbage_free((void **) &splitted_variable);
	cmd->argv = new_argv;
	return (true);
}
