...
void	manage_first_join(char *arg, char **new_argv, char **splitted_variable, t_expander *utils)
{
	char	*before_dollar;
	...
	{
		new_argv[utils->i_new] = ft_strjoin(before_dollar, splitted_variable[utils->i_split], "");
		++utils->i_new;
		++utils->i_split;
...
void	manage_last_join(char *arg, char **new_argv, char **splitted_variable, t_expander *utils)
{
	char	*after_variable;
	...
	if (utils->last_joined == true && *after_variable != '\0')
	{
		...
		new_argv[utils->i_new - 1] = ft_strjoin(splitted_variable[utils->i_split - 1], after_variable, "");
		++utils->i_split;
	}
